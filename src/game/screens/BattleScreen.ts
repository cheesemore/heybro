...
import {
  ...
  GAME_WIDTH,
  LAYOUT_SCALE,
  ARENA_PVP_BATTLE_SECONDS,
  NORMAL_BATTLE_SECONDS,
} from '../constants';
import { createSeededRandom } from '../battleRng';
...
import {
  ...
  clampBattleSpawnXY,
  defenderBattleSlotCenter,
  defenderHeroDeployBattleSlot,
...
import {
  ...
  ARCHER_BOND10_KITE_RETREAT_MARGIN_DESIGN,
  ...
  KNIGHT_BOND3_CHARGE_DAMAGE_RETAIN,
  KNIGHT_BOND3_CHARGE_DR_TAIL_SEC,
  KNIGHT_CHARGE_COOLDOWN_SEC,
  KNIGHT_CHARGE_HIT_DAMAGE_MULT,
  ...
  SHAMAN_BOND_OPENING_BLOODLUST_DELAY_SEC,
...
/**
 * ?? `effectiveSkillRangeTo` ??????`meleeEngagementMarginPx` ??????????
 * ?????????????????????????? reach???? `travel?0` ? `u.cd` ?????????????
 */
const DEFAULT_ATTACK_RANGE_SLACK_PX = Math.max(5, Math.round(6 * LAYOUT_SCALE));

/** ? HUD ???????? `mountStretchedBattleFloorBackground` ????? */
...
/** ?????????????????????? */
const VANISH_AMBUSH_MAX_CHAIN = 3;
/** ???????????????????720 ?????? */
const JETPACK_MIN_WAYPOINT_DESIGN = 300;
/** ??????????? = ???? ? ??????? */
const OVERLOAD_EXPLOSION_CHANNEL_SHIELD_MAX_HP_RATIO = 0.15;
/** ???????????????????? */
const OVERLOAD_EXPLOSION_BREAK_STUN_SEC = 5;
/** ????????????????????????skills.json params ?? 5 ?? */
...
/** ?????????????????????????720 ????? */
const ASSASSIN_BLINK_BEHIND_DIST_DESIGN = 72;
...
function artifactBuffsForLineupSlot(
  artifactBySlot: readonly (import('../strategyTypes').ArtifactKind | null)[],
  slot: number,
): { hpMult: number; atkMult: number; crit: number } {
  let hpMult = 1;
  let atkMult = 1;
  let crit = 0;
  for (let g = 0; g < 9; g++) {
    const art = artifactBySlot[g];
    if (!art) continue;
    if (art === 'holy_grail') {
      const above = g - 3;
...
function artifactBuffsForAllySlot(run: RunState, slot: number): { hpMult: number; atkMult: number; crit: number } {
  return artifactBuffsForLineupSlot(run.artifactBySlot, slot);
}
...
type SimUnit = {
  ...
  side: 'ally' | 'enemy';
  /** ??? PvP??????? `side` ?????? `side` ? enemy? */
  arenaPvpSide?: 'attacker' | 'defender';
  /** ?????????????????????????????????`hitRadiusPx` ??? */
  x: number;
  y: number;
  hp: number;
  ...
  /** ????/?????720 ??????????? `weaponReachPx` / `effectiveSkillRangeTo`?? LAYOUT_SCALE ???? `hitRadiusPx`? */
  range: number;
  speed: number;
  /** ?????????????????? */
  speedBase: number;
  cd: number;
  dead: boolean;
  /** ???????? LAYOUT_SCALE?????? `hitRadius`??? `range`?720 ?????? `weaponReachPx`????? `effectiveSkillRangeTo` */
  hitRadiusPx: number;
  root: Container;
  body: Container;
  hpRingCur?: Graphics;
  ...
  /** ???? id?`config/skills.json`???????? `wowBookBosses`????? `wowBookMonsters` */
  skillIds: string[];
  /** ?????????????????? id?????????????????? */
  ...
  /** ?????????????????? */
  druidBattleReviveCharges?: number;
  /** ???????????????????????? */
  druidReviveVanishDeath?: boolean;
  /** ?? */
  assassinBlinkStunNext?: boolean;
  /** ??????????? */
  assassinBlinkCdRem?: number;
  /** ???????????????? */
  assassinVanishHealAcc?: number;
  ...
  /** skill_shadow_bot????????????0 ??????? 0 */
  shadowBoltCdRem?: number;
  /** skill_evil_strenth??????? / ??????????? 0?????? */
...
export class BattleScreen extends Container {
  ...
  /** ????????????????? */
  private readonly defenderBondStacks: ReturnType<typeof allBondStacks> | null;
  /** ????????????????? */
  private meteorCd = 999;
  private meteorCdDefender = 999;
  private shamanBondOpeningBloodlustDelayRemDefender = 0;
  ...
  private druidRevivePillars: DruidBattleRevivePillarFx[] = [];
  ...
  /** ??????????????????????0 ????????? */
  private shamanBondOpeningBloodlustDelayRem = 0;
  /** ???????????? unitId?????????????? */
  private assassinBlinkClaimedTargetIds = new Set<number>();
  private readonly countdownGiant: Text;
  /** ??????????????? Math.random */
  private battleRand: () => number = () => Math.random();
  ...
  constructor(app: Application, run: RunState, meta: RoundMeta, onEnd: (outcome: BattleOutcome) => void) {
    ...
    this.meta = meta;
    this.onEnd = onEnd;
    const arenaSeed = run.arenaBattleRules?.battleSeed;
    if (arenaSeed != null) {
      this.battleRand = createSeededRandom(arenaSeed);
    }
    ...
    const arenaDef = run.arenaBattleRules?.defenderLineup ?? null;
    this.defenderBondStacks = arenaDef ? allBondStacks(arenaDef.board) : null;
    this.meteorCd = this.bondStacks.mage >= 15 ? MAGE_BOND_METEOR_INTERVAL_SEC : 999;
    this.meteorCdDefender =
      this.defenderBondStacks && this.defenderBondStacks.mage >= 15 ? MAGE_BOND_METEOR_INTERVAL_SEC : 999;

    const isArenaPvp = !!arenaDef;
    this.timeLimit = isArenaPvp
      ? ARENA_PVP_BATTLE_SECONDS
      : meta.uiTestBattle
        ? 120
    ...
    for (let i = 0; i < 130; i++) {
      const sx = this.battleRand() * GAME_WIDTH;
      const sy = Math.round(170 * LAYOUT_SCALE) + this.battleRand() * (GAME_HEIGHT - Math.round(340 * LAYOUT_SCALE));
      const rr = (this.battleRand() * 1.6 + 0.25) * LAYOUT_SCALE;
      stars.circle(sx, sy, rr).fill({ color: 0xe2e8f0, alpha: 0.035 + this.battleRand() * 0.08 });
    }
    this.addChild(stars);
    ...
    const arenaDefenderUnits = isArenaPvp && arenaDef ? this.spawnArenaDefender(arenaDef) : [];
    const pveEnemies = isArenaPvp ? [] : this.spawnEnemies(meta);
    this.units = [...alliesSpawned, ...heroUnits, ...arenaDefenderUnits, ...pveEnemies];
    this.applyBond25Mega(alliesSpawned);
    if (isArenaPvp && arenaDefenderUnits.length) this.applyBond25Mega(arenaDefenderUnits);
    ...
    if (isArenaPvp && arenaDef) this.applyRevengeSpiritOpeningForLineup(arenaDef, 'defender');
    ...
    this.hudEnemyLabel = new Text({
      ...
      style: {
        fontFamily: 'system-ui, Segoe UI, Roboto, sans-serif',
        fontSize: Math.round(19 * LAYOUT_SCALE),
        fill: isArenaPvp ? 0xf87171 : 0x93c5fd,
      },
    });
  ...
  /** ?? root ???????????? +Y??? `unitCircleTokens` ?????? (0,-innerR) ?? */
  ...
  private fireUiTestSkillBurst(): void {
    ...
    this.floatWords.push(spawnFloatNumber(this.floatLayer, cx, this.floatAnchorYAt(cy, refR), '????', 'buff'));
  ...
  private knockbackTargetFromAttacker(target: SimUnit, attacker: SimUnit, dist: number): void {
    ...
    if (this.battleRand() < 0.55) {
      this.hitSparks.push(spawnHitSparkBurst(this.fxLayer, target.x, target.y));
    }
    this.floatWords.push(
      spawnFloatNumber(this.floatLayer, target.x, this.floatAnchorY(target) - 36, '??', 'magic'),
    );
  }
  ...
  private maybeDarkspearKnockback(attacker: SimUnit, target: SimUnit): void {
    ...
    if (this.battleRand() >= skillParamNumber(ds, 0, 0.1)) return;
    ...
    this.floatWords.push(
      spawnFloatNumber(this.floatLayer, target.x, this.floatAnchorY(target) - 48, '??', 'magic'),
    );
  }
  ...
  private rhahkCleaveFollowup(attacker: SimUnit, primary: SimUnit): void {
    ...
    if (this.battleRand() >= RHAKH_CLEAVE_PROC_CHANCE) return;
    ...
    if (hits > 0) {
      this.logBattleSkill('skill_rhahk_cleave', attacker, `??????=${hits}`);
    }
  }
  ...
  private knockbackAllyRadialFromPoint(a: SimUnit, cx: number, cy: number, distPx: number): void {
    ...
    const nx = dx / d;
    const ny = dy / d;
    const want = distPx * (0.9 + this.battleRand() * 0.25);
  ...
  private applyBloodlustBuff(_caster: SimUnit, recipient: SimUnit): void {
    ...
    this.floatWords.push(
      spawnFloatNumber(this.floatLayer, recipient.x, this.floatAnchorY(recipient) - 40, '???', 'buff'),
    );
    for (let k = 0; k < 3; k++) {
      this.ringFx.push(
      ...
      if (b.acc >= b.tickSec) {
        ...
        for (const a of this.alive('ally')) {
          if (Math.hypot(a.x - b.patch.x, a.y - b.patch.y) <= b.r) {
            ...
            if (this.battleRand() < CATAPULT_BURN_SPARK_CHANCE) this.hitSparks.push(spawnHitSparkBurst(this.fxLayer, a.x, a.y));
          }
        }
      }
  ...
  private applyUnitCollisionSeparation(iterations: number): void {
    ...
    for (let it = 0; it < iterations; it++) {
      ...
      for (let i = 0; i < alive.length; i++) {
        for (let j = i + 1; j < alive.length; j++) {
          ...
          const nx = dx / dist;
          const ny = dy / dist;
          const aKb = !!a.knockbackTween;
          const bKb = !!b.knockbackTween;
    ...
    for (const u of this.units) {
      if (!u.dead && !u.knockbackTween) clampOne(u);
    }
  }
  ...
  private battleGearStatMult(): number {
    if (this.run.arenaBattleRules) return gsCombatStatMult(0);
  ...
  private battleClassProgressMult(cls: AllyClass): number {
    if (this.run.arenaBattleRules) return 1;
    return classLevelStatMult(getClassLevel(cls));
  }

  private get isArenaPvp(): boolean {
    return this.defenderBondStacks != null;
  }

  private pvpTeam(u: SimUnit): 'attacker' | 'defender' {
    if (u.arenaPvpSide) return u.arenaPvpSide;
    return u.side === 'ally' ? 'attacker' : 'defender';
  }

  private bondStacksFor(sideOrUnit: SimUnit | 'attacker' | 'defender'): ReturnType<typeof allBondStacks> {
    const team =
      sideOrUnit === 'attacker' || sideOrUnit === 'defender'
        ? sideOrUnit
        : this.pvpTeam(sideOrUnit);
    if (!this.isArenaPvp || team === 'attacker') return this.bondStacks;
    return this.defenderBondStacks!;
  }

  private isHostileUnits(a: SimUnit, b: SimUnit): boolean {
    return this.pvpTeam(a) !== this.pvpTeam(b);
  }

  /** ?????? AI???/??/??/????????????? */
  private usesAllyClassAI(u: SimUnit): boolean {
    if (u.side === 'ally') return true;
    return this.isArenaPvp && u.arenaPvpSide === 'defender' && u.allyKind != null;
  }

  /** ????/?? AI???????????????? */
  private isArenaNpcEnemy(u: SimUnit): boolean {
    return u.side === 'enemy' && (!this.isArenaPvp || u.arenaPvpSide !== 'defender');
  }

  private aliveOnTeam(team: 'attacker' | 'defender'): SimUnit[] {
    return this.units.filter((u) => !u.dead && this.pvpTeam(u) === team);
  }

  private aliveHostileTo(from: SimUnit): SimUnit[] {
    return this.units.filter((u) => !u.dead && this.isHostileUnits(from, u));
  }

  private spawnAllies(): SimUnit[] {
    const out: SimUnit[] = [];
    const bondStacks = this.bondStacksFor('attacker');
    const teamM =
      priestBondTeamMultiplier(bondStacks.priest) *
      shamanBondTeamMultiplier(bondStacks.shaman) *
      druidBondTeamMultiplier(bondStacks.druid);

    for (let slot = 0; slot < 9; slot++) {
      const cell = this.run.board[slot];
      if (!cell) continue;
      const def = ALLY_DEFS[cell.kind];
      const cellStacks = Math.min(cell.stacks, BOARD_CELL_MAX_STACKS);
      const classM = classBondHpAtkMultiplier(bondStacks[cell.kind]);
      const mult = classM * teamM;
      const ab = artifactBuffsForAllySlot(this.run, slot);

      const n = Math.max(1, cellStacks);
      /** ?????????????????????????????? */
      ...
      const arenaFair = !!this.run.arenaBattleRules;
      const growHp = arenaFair ? 1 : this.run.externalGrowth.permanentMaxHpMult;
      const growAtk = arenaFair ? 1 : this.run.externalGrowth.permanentDamageMult;
      const gearM = this.battleGearStatMult();
      const classProg = this.battleClassProgressMult(cell.kind);
      ...
      let range = def.range;
      if (cell.kind === 'archer') {
        range += Math.round(archerBondRangeBonusDesign(bondStacks.archer) * LAYOUT_SCALE);
      }
        ...
        if (cell.kind === 'knight') {
          out.push(
            this.makeUnit('ally', x, y, eachHp, eachAtk, def.attackSpeed, def.range, def.moveSpeed, def.name, {
              allyKind: 'knight',
              arenaPvpSide: this.isArenaPvp ? 'attacker' : undefined,
              knightState: 'fight',
              knightCooldown: 0,
              knightChargeTargetId: null,
              knightDeathDenyLeft: bondStacks.knight >= 15 ? 1 : 0,
              allySourceSlot: slot,
              bonusCrit: ab.crit,
  ...
  /** ???????????????????????????? */
  private spawnArenaDefender(lineup: ArenaLineupSnapshot): SimUnit[] {
    const minions = this.spawnArenaDefenderMinions(lineup);
    const heroes = this.spawnArenaDefenderHeroes(lineup, minions);
    return [...minions, ...heroes];
  }

  private spawnArenaDefenderMinions(lineup: ArenaLineupSnapshot): SimUnit[] {
    const out: SimUnit[] = [];
    const stacks = this.bondStacksFor('defender');
    const teamM =
      priestBondTeamMultiplier(stacks.priest) *
      shamanBondTeamMultiplier(stacks.shaman) *
      druidBondTeamMultiplier(stacks.druid);
    ...
    for (let slot = 0; slot < 9; slot++) {
      ...
      const def = ALLY_DEFS[cell.kind];
      const cellStacks = Math.min(cell.stacks, BOARD_CELL_MAX_STACKS);
      const classM = classBondHpAtkMultiplier(stacks[cell.kind]);
      const mult = classM * teamM;
      const ab = artifactBuffsForLineupSlot(lineup.artifactBySlot, slot);
      const n = Math.max(1, cellStacks);
      const gearM = gsCombatStatMult(0);
      const classProg = 1;
      const eachHp = Math.max(1, Math.round(def.maxHp * mult * ab.hpMult * gearM * classProg));
      const eachAtk = Math.max(1, Math.round(def.atk * mult * ab.atkMult * gearM * classProg));
      let range = def.range;
      if (cell.kind === 'archer') {
        range += Math.round(archerBondRangeBonusDesign(stacks.archer) * LAYOUT_SCALE);
      }
      for (let i = 0; i < n; i++) {
        const { x, y } = this.defenderSubSpawnXY(slot, i, n);
        if (cell.kind === 'knight') {
          out.push(
            this.makeUnit('enemy', x, y, eachHp, eachAtk, def.attackSpeed, def.range, def.moveSpeed, def.name, {
              allyKind: 'knight',
              arenaPvpSide: 'defender',
              knightState: 'fight',
              knightCooldown: 0,
              knightChargeTargetId: null,
              knightDeathDenyLeft: stacks.knight >= 15 ? 1 : 0,
              allySourceSlot: slot,
              bonusCrit: ab.crit,
              hitRadiusDesign: def.hitRadius,
            }),
          ...
          out.push(
            this.makeUnit('enemy', x, y, eachHp, eachAtk, def.attackSpeed, range, def.moveSpeed, def.name, {
              allyKind: cell.kind,
              arenaPvpSide: 'defender',
              allySourceSlot: slot,
              bonusCrit: ab.crit,
  ...
  private defenderSubSpawnXY(slot: number, index: number, n: number): { x: number; y: number } {
    const { x: cx, y: cy } = defenderBattleSlotCenter(slot);
    const sc = this.scatterOffset(slot * 19 + index * 11 + n * 5);
    if (n <= 1) {
      return { x: cx + sc.jx * 0.22, y: cy + sc.jy * 0.22 };
    }
    ...
    const stepY = Math.round(28 * LAYOUT_SCALE);
    const ox = (colI - (cols - 1) / 2) * stepX;
    const oy = (rowI - (Math.ceil(n / cols) - 1) / 2) * stepY;
    return { x: cx + ox + sc.jx * 0.18, y: cy + oy + sc.jy * 0.18 };
  }

  private spawnArenaDefenderHeroes(lineup: ArenaLineupSnapshot, boardUnits: SimUnit[]): SimUnit[] {
    const stacks = this.bondStacksFor('defender');
    const deploy = lineup.heroDeploy;
    const teamM =
      priestBondTeamMultiplier(stacks.priest) *
      shamanBondTeamMultiplier(stacks.shaman) *
      druidBondTeamMultiplier(stacks.druid);
    const gearM = gsCombatStatMult(0);
    const classProgFor = (_cls: AllyClass): number => 1;
    const out: SimUnit[] = [];
    const cap = Math.min(3, deploy.length);
    for (let s = 0; s < cap; s++) {
      const hid = deploy[s];
      if (!hid) continue;
      ...
      const classM = classBondHpAtkMultiplier(stacks[cls]);
      const mult = classM * teamM;
      const cProg = classProgFor(cls);
      let eachHp = Math.max(1, Math.round(hd.maxHp * starM * mult * gearM * cProg));
      if (hid === PRIEST_SHELTER_BLUE_ID) eachHp = Math.max(1, Math.round(eachHp * 1.1));
      let eachAtk = Math.max(1, Math.round(hd.atk * starM * mult * gearM * cProg));
      if (hid === KNIGHT_HOLY_SANCTION_BLUE_ID) eachAtk = Math.max(1, Math.round(eachAtk * 1.1));
      let range = hd.range;
      if (cls === 'archer') {
        range += Math.round(archerBondRangeBonusDesign(stacks.archer) * LAYOUT_SCALE);
      }
      ...
      const { x: sx, y: sy } = this.defenderHeroSpawnXY(boardUnits, cls, s);
      if (cls === 'knight') {
        out.push(
          this.makeUnit('enemy', sx, sy, eachHp, eachAtk, hd.attackSpeed, hd.range, hd.moveSpeed, hd.name, {
            allyKind: 'knight',
            arenaPvpSide: 'defender',
            knightState: 'fight',
            knightCooldown: 0,
        ...
        out.push(
          this.makeUnit('enemy', sx, sy, eachHp, eachAtk, hd.attackSpeed, range, hd.moveSpeed, hd.name, {
            allyKind: cls,
            arenaPvpSide: 'defender',
            bonusCrit: heroBonusCrit,
            hitRadiusDesign: hd.hitRadius,
  ...
  private defenderHeroSpawnXY(
    boardUnits: SimUnit[],
    cls: AllyClass,
    deploySlotIndex: number,
  ): { x: number; y: number } {
    const mates = boardUnits.filter(
      (u) => u.arenaPvpSide === 'defender' && !u.heroId && u.allyKind === cls,
    );
    if (mates.length > 0) {
    ...
    const slot = defenderHeroDeployBattleSlot(deploySlotIndex);
    const { x, y } = defenderBattleSlotCenter(slot);
  ...
  private heroDeployedSpawnXY(
    ...
    const mates = boardAllies.filter(
      (u) => u.arenaPvpSide !== 'defender' && u.side === 'ally' && !u.heroId && u.allyKind === cls,
    );
    if (mates.length > 0) {
      const cx = mates.reduce((s, u) => s + u.x, 0) / mates.length;
      const cy = mates.reduce((s, u) => s + u.y, 0) / mates.length;
  ...
  private spawnDeployedHeroes(boardAllies: SimUnit[]): SimUnit[] {
    ...
    const cap = hooks
      ...
      : maxHeroDeploySlots();
    const arenaRules = this.run.arenaBattleRules;
    const deployed = arenaRules?.heroDeploy ?? hooks?.heroDeploy ?? getDeployedHeroIds();
    const meta = loadHeroMeta();
    const atkStacks = this.bondStacksFor('attacker');
    const teamM =
      priestBondTeamMultiplier(atkStacks.priest) *
      shamanBondTeamMultiplier(atkStacks.shaman) *
      druidBondTeamMultiplier(atkStacks.druid);
    const bossHpM = this.meta.kind === 'boss' ? this.run.bossHpDerivedFinalHpMult : 1;
    const bossAtkM = this.meta.kind === 'boss' ? this.run.bossHpDerivedFinalAtkMult : 1;
    const arenaFair = !!arenaRules;
    const growHp = arenaFair ? 1 : this.run.externalGrowth.permanentMaxHpMult;
    const growAtk = arenaFair ? 1 : this.run.externalGrowth.permanentDamageMult;
    const gearM = this.battleGearStatMult();
    const classProgFor = (cls: AllyClass): number => this.battleClassProgressMult(cls);
    const out: SimUnit[] = [];
    for (let s = 0; s < cap; s++) {
      ...
      const stars = arenaRules ? 1 : (entry?.stars ?? 1);
      const starM = heroStarStatMult(stars);
      const cls = hd.allyClass;
      const classM = classBondHpAtkMultiplier(atkStacks[cls]);
      const mult = classM * teamM;
      const cProg = classProgFor(cls);
      let eachHp = Math.max(
        1,
      ...
      let range = hd.range;
      if (cls === 'archer') {
        range += Math.round(archerBondRangeBonusDesign(atkStacks.archer) * LAYOUT_SCALE);
      }
      if (hid === ARCHER_STRONG_STRIKE_BLUE_ID) range += 50;
      const heroBonusCrit = hid === MAGE_ARCANE_BLUE_ID ? 0.1 : 0;
      ...
      if (cls === 'knight') {
        out.push(
          this.makeUnit('ally', sx, sy, eachHp, eachAtk, hd.attackSpeed, hd.range, hd.moveSpeed, hd.name, {
            ...
            knightCooldown: 0,
            knightChargeTargetId: null,
            knightDeathDenyLeft: atkStacks.knight >= 15 ? 1 : 0,
            bonusCrit: heroBonusCrit,
            hitRadiusDesign: hd.hitRadius,
            heroId: hid,
          }),
  ...
  private applyBondBattleOpenBuffs(units: SimUnit[]): void {
    this.applyBondBattleOpenBuffsForTeam(units, 'attacker');
    if (this.isArenaPvp) this.applyBondBattleOpenBuffsForTeam(units, 'defender');
  }

  private applyBondBattleOpenBuffsForTeam(units: SimUnit[], team: 'attacker' | 'defender'): void {
    const stacks = this.bondStacksFor(team);
    const wardHits = mageBondArcaneWardHits(stacks.mage);
    if (wardHits > 0) {
      for (const u of units) {
        if (u.dead || this.pvpTeam(u) !== team || u.allyKind !== 'mage') continue;
        u.arcaneWardHitsLeft = wardHits;
      }
    }
    if (stacks.warlock >= 10) {
      this.castWarlockOpeningFearVolley(units, team);
    }
    if (stacks.shaman >= 6) {
      if (team === 'attacker') {
        this.shamanBondOpeningBloodlustDelayRem = SHAMAN_BOND_OPENING_BLOODLUST_DELAY_SEC;
      } else {
        this.shamanBondOpeningBloodlustDelayRemDefender = SHAMAN_BOND_OPENING_BLOODLUST_DELAY_SEC;
      }
    }
    if (stacks.assassin >= 6) {
      for (const u of units) {
        if (u.dead || this.pvpTeam(u) !== team || u.allyKind !== 'assassin') continue;
        u.assassinDodgeRem = ASSASSIN_BOND6_DODGE_DURATION_SEC;
      }
    }
  }

  private mageSplashRadiusPx(forUnit?: SimUnit): number {
    const stacks = forUnit ? this.bondStacksFor(forUnit) : this.bondStacks;
    return Math.round(mageBondSplashRadiusDesign(stacks.mage) * LAYOUT_SCALE);
  }

  /** ??????????????? */
  private applyAllyStun(u: SimUnit, stunSec: number): void {
    if (stunSec <= 0 || u.dead || !this.usesAllyClassAI(u)) return;
    if (u.allyKind === 'priest' && this.bondStacksFor(u).priest >= 10) return;
  ...
  private unitImmuneToWarlockFear(u: SimUnit): boolean {
    ...
    if (this.isArenaPvp && u.arenaPvpSide === 'defender') return false;
  ...
  private pickWarlockOpeningFearTarget(casterTeam: 'attacker' | 'defender', exclude: Set<number>): SimUnit | null {
    let best: SimUnit | null = null;
    for (const e of this.units) {
      if (e.dead || this.pvpTeam(e) === casterTeam) continue;
  ...
  private castWarlockOpeningFearVolley(units: SimUnit[], team: 'attacker' | 'defender'): void {
    const warlocks = units.filter((u) => this.pvpTeam(u) === team && !u.dead && u.allyKind === 'warlock');
    if (!warlocks.length) return;
    const claimed = new Set<number>();
    for (const w of warlocks) {
      const tgt = this.pickWarlockOpeningFearTarget(team, claimed);
      if (!tgt) break;
  ...
  private applyWarlockFear(source: SimUnit, target: SimUnit): void {
    ...
    const dur = warlockBondFearDurationSec(this.bondStacksFor(source).warlock);
    if (dur <= 0) return;
  ...
  private tryWarlockFearOnHit(source: SimUnit, target: SimUnit): void {
    if (source.allyKind !== 'warlock' || target.dead || !this.isHostileUnits(source, target)) return;
    if (this.battleRand() >= WARLOCK_FEAR_PROC_CHANCE) return;
  ...
  private onWarlockAllyHit(source: SimUnit, target: SimUnit, hpLoss: number): void {
    ...
    const ls = warlockBondLifestealRatio(this.bondStacksFor(source).warlock);
    if (ls > 0) {
  ...
  private tickWarlockSoulFire(u: SimUnit, dt: number): void {
    ...
    if (this.bondStacksFor(u).warlock < 15) return;
    ...
    this.floatWords.push(
      spawnFloatNumber(this.floatLayer, u.x, this.floatAnchorY(u) - 40, '????', 'magic'),
    );
  ...
  private applyBond25Mega(allies: SimUnit[]): void {
    for (const kind of ALLY_CLASSES) {
      ...
      const team = this.pvpTeam(pool[0]!);
      if (!hasBondMega(this.bondStacksFor(team)[kind])) continue;
      if (!pool.length) continue;
      for (let i = pool.length - 1; i > 0; i--) {
        const j = Math.floor(this.battleRand() * (i + 1));
        const t = pool[i]!;
        pool[i] = pool[j]!;
        pool[j] = t;
      }
  ...
  private applyDruidBattleSetup(units: SimUnit[]): void {
    const druids = units.filter((u) => this.usesAllyClassAI(u) && !u.dead && u.allyKind === 'druid');
    if (!druids.length) return;
    const team = this.pvpTeam(druids[0]!);
    const stacks = this.bondStacksFor(team);
    ...
    if (stacks.druid >= 15) {
      for (const u of druids) {
        if (u.heroId || u.druidForm === 'caster') {
          u.druidBattleReviveCharges = 1;
        }
      }
    }

    if (hasBondMega(stacks.druid)) {
  ...
  private castShamanBondOpeningBloodlust(units: SimUnit[], team: 'attacker' | 'defender'): void {
    const shamanStacks = this.bondStacksFor(team).shaman;
    for (const u of units) {
      if (u.dead || this.pvpTeam(u) !== team) continue;
      this.applyAllyBondBloodlustBuff(u, shamanStacks);
    }
  }

  /** ????????????????????????????/????? */
  private tryCastShamanHealWave(shaman: SimUnit): boolean {
    const shStacks = this.bondStacksFor(shaman).shaman;
    const n = shamanBondHealWaveTargets(shStacks);
    const ranked = this.aliveOnTeam(this.pvpTeam(shaman))
    ...
    for (const { a } of picks) {
      ...
      healedIds.push(a.unitId);
    }
    if (healedIds.length) this.maybeShamanWindfury(shaman, 'heal', healedIds, heal);
    return true;
  }
  ...
  private castShamanLightningBolt(shaman: SimUnit, target: SimUnit): void {
    ...
    this.queueProjectile(
      ...
      () => {
        ...
        this.maybeShamanWindfury(a, 'damage', [tid], dmg);
      },
      { style: 'ally_shaman_lightning' },
    );
  }

  private maybeShamanWindfury(
    shaman: SimUnit,
    kind: 'damage' | 'heal',
    targetIds: number[],
    amount: number,
  ): void {
    const chance = shamanBondWindfuryChance(this.bondStacksFor(shaman).shaman);
    if (chance <= 0 || this.battleRand() >= chance || !targetIds.length) return;
    this.floatWords.push(
      spawnFloatNumber(this.floatLayer, shaman.x, this.floatAnchorY(shaman) - 52, '??', 'buff'),
    );
    shaman.shamanWindfuryPending = {
      ...
      targetIds: [...targetIds],
      amount,
      shamanId: shaman.unitId,
    };
  }

  private fireShamanWindfuryHit(shaman: SimUnit, w: NonNullable<SimUnit['shamanWindfuryPending']>): void {
    if (w.kind === 'damage') {
      const tid = w.targetIds[0];
      if (tid == null) return;
      const uid = shaman.unitId;
      const dmg = w.amount;
      this.queueProjectile(
    ...
    for (const tid of w.targetIds) {
      const t = this.byId(tid);
      if (!t || t.dead) continue;
      this.applyHeal(t, w.amount, shaman, { silent: true });
    }
  }
  ...
  private tickShamanWindfuryPending(u: SimUnit, dt: number): void {
    ...
    const shaman = this.units.find((x) => x.unitId === w.shamanId && !x.dead);
    if (!shaman) {
      u.shamanWindfuryPending = undefined;
      return;
    }
    const liveTargets = w.targetIds.filter((id) => {
    ...
    w.targetIds = liveTargets;
    this.fireShamanWindfuryHit(shaman, w);
    w.hitsLeft -= 1;
    if (w.kind === 'damage') {
      const t = this.byId(liveTargets[0]!);
      if (t) this.hitSparks.push(spawnHitSparkBurst(this.fxLayer, t.x, t.y));
    }
  }
  ...
  private maybeDruidBearSwipe(bear: SimUnit, primary: SimUnit): void {
    if (bear.druidForm !== 'bear' || this.bondStacksFor(bear).druid < 6) return;
    ...
    let hits = 0;
    for (const e of this.aliveHostileTo(bear)) {
    ...
    if (hits > 0) {
      this.floatWords.push(
        spawnFloatNumber(this.floatLayer, bear.x, this.floatAnchorY(bear) - 36, '??', 'magic'),
      );
    }
  }

  private pickRejuvenationTarget(caster: SimUnit): SimUnit | null {
    const allies = this.aliveOnTeam(this.pvpTeam(caster));
  ...
  private tickDruidCasterRejuvenation(u: SimUnit, dt: number): void {
    if (u.druidForm !== 'caster' || this.bondStacksFor(u).druid < 10) return;
    ...
    this.floatWords.push(
      spawnFloatNumber(this.floatLayer, tgt.x, this.floatAnchorY(tgt) - 36, '???', 'heal'),
    );
  ...
  private assassinMeleeEnemyInRange(u: SimUnit): boolean {
    for (const e of this.aliveHostileTo(u)) {
  ...
  /** ???????????? ? ????????????????????? */
  private assassinBlinkLandingXY(assassin: SimUnit, target: SimUnit): { x: number; y: number } {
    const allies = this.aliveOnTeam(this.pvpTeam(assassin));
    let ax = GAME_WIDTH * 0.5;
    let ay = this.pvpTeam(assassin) === 'defender' ? ARENA_Y_MIN : ARENA_Y_MAX;
    if (allies.length) {
      ax = allies.reduce((s, a) => s + a.x, 0) / allies.length;
      ay = allies.reduce((s, a) => s + a.y, 0) / allies.length;
    }
    let bx = target.x - ax;
    let by = target.y - ay;
    ...
    const backDist = Math.max(
      Math.round(ASSASSIN_BLINK_BEHIND_DIST_DESIGN * LAYOUT_SCALE),
      this.effectiveSkillRangeTo(assassin, target) * 0.45,
      assassin.hitRadiusPx + target.hitRadiusPx + Math.round(8 * LAYOUT_SCALE),
    );
    const perpX = -by;
    const perpY = bx;
    const slot = assassin.allySourceSlot ?? assassin.unitId;
    const lateral = ((slot % 3) - 1) * Math.round(18 * LAYOUT_SCALE);
    return this.clampBattleSpawnXY(
      target.x + bx * backDist + perpX * lateral,
      target.y + by * backDist + perpY * lateral,
    );
  }

  private pickAssassinBlinkTarget(u: SimUnit): SimUnit | null {
    const enemies = this.units.filter((e) => !e.dead && this.isHostileUnits(u, e));
    if (!enemies.length) return null;
    const ranged = enemies.filter((e) => e.range >= RANGED_ATTACK_RANGE_THRESHOLD);
    const pool = ranged.length ? ranged : enemies;
    let candidates = pool.filter((e) => !this.assassinBlinkClaimedTargetIds.has(e.unitId));
    if (!candidates.length) candidates = [...pool];
    return candidates[Math.floor(this.battleRand() * candidates.length)]!;
  }

  private tryAssassinBlinkStrike(u: SimUnit, opts?: { force?: boolean }): boolean {
    if (u.allyKind !== 'assassin' || u.assassinVanishPhase) return false;
    if (!opts?.force && (u.assassinBlinkCdRem ?? 0) > 0) return false;
    if (this.assassinMeleeEnemyInRange(u)) return false;
    const tgt = this.pickAssassinBlinkTarget(u);
    if (!tgt) return false;
    this.assassinBlinkClaimedTargetIds.add(tgt.unitId);
    const land = this.assassinBlinkLandingXY(u, tgt);
    u.x = land.x;
    u.y = land.y;
    this.syncUnitRootFromStance(u);
    u.assassinBlinkCdRem = ASSASSIN_BLINK_CD_SEC;
    this.ringFx.push(spawnRingPulse(this.fxLayer, u.x, u.y, u.hitRadiusPx * 1.4, 0xfff569, 0.45));
    this.floatWords.push(
      spawnFloatNumber(this.floatLayer, u.x, this.floatAnchorY(u) - 32, '??', 'magic'),
    );
    if (this.bondStacksFor(u).assassin >= 10) u.assassinBlinkStunNext = true;
    return true;
  }
  ...
  private tickAssassinVanish(u: SimUnit, dt: number): boolean {
    ...
    u.assassinVanishHealAcc = (u.assassinVanishHealAcc ?? 0) + dt;
    while ((u.assassinVanishHealAcc ?? 0) >= 1) {
      u.assassinVanishHealAcc -= 1;
      const heal = Math.max(1, Math.round(u.maxHp * ASSASSIN_VANISH_HEAL_MAX_HP_PER_SEC));
      this.applyHeal(u, heal, u);
    }
    if ((u.assassinVanishHoldRem ?? 0) <= 0) {
      u.assassinVanishPhase = undefined;
      u.assassinVanishHealAcc = undefined;
      u.invincible = false;
      u.root.alpha = 1;
      this.tryAssassinBlinkStrike(u, { force: true });
    }
    return true;
  }

  private tryAssassinVanishOnLowHp(u: SimUnit): void {
    if (u.allyKind !== 'assassin' || u.assassinVanishUsed || this.bondStacksFor(u).assassin < 15) return;
    ...
    u.assassinVanishHealAcc = 0;
    u.invincible = true;
    this.floatWords.push(
      spawnFloatNumber(this.floatLayer, u.x, this.floatAnchorY(u) - 48, '??', 'buff'),
    );
  }
  ...
  private jitterChaosSpawnPositions(): void {
    ...
    for (const u of this.units) {
      const ir = this.unitTokenRootYOffsetPx(u);
      u.x = xmin + this.battleRand() * (xmax - xmin);
      u.y = ymin + this.battleRand() * (ymax - ymin) - ir;
      this.syncUnitRootFromStance(u);
    }
  }
  ...
  private applyRevengeSpiritOpening(): void {
    this.applyRevengeSpiritOpeningForLineup(
      { board: this.run.board, artifactBySlot: this.run.artifactBySlot, heroDeploy: [] },
      'attacker',
    );
  }

  private applyRevengeSpiritOpeningForLineup(
    lineup: ArenaLineupSnapshot,
    ownerTeam: 'attacker' | 'defender',
  ): void {
    for (let g = 0; g < 9; g++) {
      if (lineup.artifactBySlot[g] !== 'revenge_spirit') continue;
      let links = 0;
      ...
      for (const d of deltas) {
        ...
        if (!lineup.board[n]) continue;
        links += 1;
        const cut = 0.2;
        for (const u of this.units) {
          if (u.dead || this.pvpTeam(u) !== ownerTeam) continue;
      ...
      const pct = 0.06 * L;
      for (const e of this.units) {
        if (e.dead || this.pvpTeam(e) === ownerTeam) continue;
  ...
  private makeUnit(
    ...
    opts: {
      ...
      /**
       * ??????
       * - ?? `wowMobId`?`enemySkillIds` ?????????? `skillIds`??? = `[]`??**???** `ENEMY_DEFS[enemyPaint]`?
       * - ? `enemyPaint`???? id??`enemySkillIds` ???? `ENEMY_DEFS[enemyPaint].skillIds`?? `wowBookMonsters` ??? id ?????
       */
      enemySkillIds?: string[];
      /**
       * ?????`resolveWowBookBossCombat` ???? `skillIds`??????? `bossSkillIds` ?????????
       */
      bossSkillIds?: string[];
      arenaPvpSide?: 'attacker' | 'defender';
    } = {},
    ...
    } else if (side === 'enemy' && opts.allyKind && opts.arenaPvpSide === 'defender') {
      const parts = opts.heroId
        ? createBattleHeroToken(opts.heroId, opts.allyKind, hitRadiusPx)
        : createBattleAllyToken(opts.allyKind, hitRadiusPx);
      body = parts.root;
    ...
    if (opts.heroId) {
      ...
      if (hd) {
        const bondStacks = opts.arenaPvpSide
          ? this.bondStacksFor(opts.arenaPvpSide)
          : this.bondStacks;
        const nameStr = heroDisplayNameWithSkillTier(hd.name, bondStacks[hd.allyClass]);
    ...
    const isPlayerHero =
      opts.heroId &&
      (side === 'ally' || (side === 'enemy' && opts.arenaPvpSide === 'defender'));
    let skillIds: string[] = [];
    if (isPlayerHero && isMageArcaneMissilesHero(opts.heroId!)) {
      skillIds = ['skill_hero_mage_arcane_missiles'];
    } else if (isPlayerHero && isPriestMassShelterHero(opts.heroId!)) {
      skillIds = ['skill_hero_priest_mass_shelter'];
    } else if (isPlayerHero && isArcherSnareTrapHero(opts.heroId!)) {
      skillIds = [SKILL_HERO_ARCHER_SNARE_TRAP];
    } else if (isPlayerHero && isKnightHolySanctionHero(opts.heroId!)) {
      skillIds = ['skill_hero_knight_holy_sanction'];
    } else if (side === 'enemy' && opts.arenaPvpSide !== 'defender') {
      if (opts.bossId) {
    ...
    const isNpcEnemy = side === 'enemy' && opts.arenaPvpSide !== 'defender';
    const u: SimUnit = {
      ...
      enemyLeapCd:
        isNpcEnemy && skillIds.some((id) => (MINION_LEAP_SKILL_IDS as readonly string[]).includes(id))
          ? 1.2 + this.battleRand() * 1.8
          : undefined,
      shamanBloodlustCd:
        isNpcEnemy && skillIds.includes('skill_shaman_bloodlust')
          ? this.battleRand() * skillParamNumber(getSkillById('skill_shaman_bloodlust'), 0, 6) * (2.2 / 6)
          : undefined,
      dreadAssaultUsed: isNpcEnemy && skillIds.includes('skill_dread_warrior_assault') ? false : undefined,
      shadowBoltCdRem: isNpcEnemy && skillIds.includes('skill_shadow_bot') ? 0 : undefined,
      evilFrenzyCdRem: isNpcEnemy && skillIds.includes('skill_evil_strenth') ? 0 : undefined,
      evilFrenzyBuffT: isNpcEnemy && skillIds.includes('skill_evil_strenth') ? 0 : undefined,
      enemyHeavyStunCdRem: isNpcEnemy && skillIds.includes(SKILL_ENEMY_HEAVY_STUN) ? 0 : undefined,
      enemyBombCdRem:
        isNpcEnemy && skillIds.includes(SKILL_ELASTIC_BOMB)
          ? skillParamNumber(getSkillById(SKILL_ELASTIC_BOMB), 1, 3)
          : undefined,
      voidWalkCdRem: isNpcEnemy && skillIds.includes(SKILL_VOID_WALK) ? 0 : undefined,
      heroId: opts.heroId,
      arenaPvpSide: opts.arenaPvpSide,
    };
    if (isNpcEnemy && skillIds.includes('skill_berserker')) {
      u.bossBerserkBaseAtk = atk;
      u.bossBerserkStage = 0;
      u.bossBerserkSparkAcc = 0;
    }
    if (isNpcEnemy && skillIds.includes('skill_rhahk_warcry')) {
  ...
  private buildDeathLaunch(sx: number): {
    ...
    const vx0 = sign * (360 + this.battleRand() * 420) * s + (this.battleRand() - 0.5) * 140 * s;
    const vy0 = -(1050 + this.battleRand() * 720) * s;
    const g = (1850 + this.battleRand() * 580) * s;
    const maxT = DEATH_FLIGHT_MAX_T + this.battleRand() * 0.85;
  ...
  private tickDeathAnimations(dt: number): void {
    ...
    for (const u of this.units) {
      if (u.dead && (u.boomSkipDeathAnim || u.druidReviveVanishDeath) && !u.deathAnim) boomRemove.push(u);
    }
  ...
  private tickDefiasBandage(u: SimUnit, dt: number): boolean {
    ...
    if (ch) {
      ...
      const dur = skillParamNumber(def, 2, 6);
      const pct = skillParamNumber(def, 1, 10);
      ch.t += dt;
  ...
  private tickEnemyBacklineLeap(u: SimUnit, dt: number): boolean {
    ...
    const jx = (this.battleRand() - 0.5) * Math.round(150 * LAYOUT_SCALE);
    ...
    u.enemyLeapCd = baseCd + this.battleRand() * 2.2;
    const leapSkillId =
  ...
  /**
   * ? `logDevBattleTest` ? sink??? `RunState.devBattleTestLog`???/????????????
   * ??? `[battle-skill]`?????? `nameCn` + ?? `eventId`?
   */
  ...
  /** `skillId` ???? `skills.json`???? `nameCn`????? `logBattleEvent`? */
  ...
  /** skill_defias_fever????????? skill_defias ????? params[1]????????? */
  ...
  private tickEnemyElasticBombTry(u: SimUnit, dt: number): void {
    ...
    const tgt = allies[Math.floor(this.battleRand() * allies.length)]!;
  ...
  private castEnemyElasticBomb(attacker: SimUnit, target: SimUnit): void {
    ...
    const onBombHit = () => {
      ...
      for (const ally of inSplash) {
        this.knockbackAllyFromPoint(ally, cx, cy, kbDist);
      }
  ...
  private tickEnemyVoidWalk(u: SimUnit, dt: number): boolean {
    ...
    if (dash) {
      ...
      if (dist <= stepLen + Math.round(6 * LAYOUT_SCALE)) {
        ...
        this.logBattleSkill(SKILL_VOID_WALK, u, tgt ? `?? #${tgt.unitId}` : '??');
      } else {
        u.x += (dx / dist) * stepLen;
        u.y += (dy / dist) * stepLen;
        this.syncUnitRootFromStance(u);
    ...
    const tgt = ranged[Math.floor(this.battleRand() * ranged.length)]!;
  ...
  private spawnChapterMobSummonWave(cx: number, cy: number, count: number, groupTag: number): void {
    ...
    for (let i = 0; i < count; i++) {
      const id = pool[Math.floor(this.battleRand() * pool.length)]!;
  ...
  private tickAllyRfc3CorrosionDot(u: SimUnit, dt: number): void {
    ...
    u.rfc3CorrosionTickAcc = (u.rfc3CorrosionTickAcc ?? 0) + dt;
    if ((u.rfc3CorrosionTickAcc ?? 0) >= 1) {
      u.rfc3CorrosionTickAcc = (u.rfc3CorrosionTickAcc ?? 0) - 1;
      const dmg = Math.max(1, Math.round(dps));
      this.applyDamage(u, dmg, { attacker: boss ?? undefined, damageTag: 'magic' });
    }
  ...
  /** ?????????????????????????????????????????? */
  private tryDruidBattleRevive(victim: SimUnit): boolean {
    if (this.bondStacks.druid < 15) return false;
    if (victim.side !== 'ally' || victim.dead || !victim.allyKind) return false;

    const victimIsHero = !!victim.heroId;
    const revivers = this.alive('ally').filter((d) => {
      if (d.allyKind !== 'druid' || (d.druidBattleReviveCharges ?? 0) <= 0) return false;
      if (d.unitId === victim.unitId) return false;
      const reviverIsHero = !!d.heroId;
      if (reviverIsHero !== victimIsHero) return false;
      if (!reviverIsHero && d.druidForm !== 'caster') return false;
      return true;
    });
    if (!revivers.length) return false;

    const reviver = revivers.reduce((best, d) => {
      const db = (best.x - victim.x) ** 2 + (best.y - victim.y) ** 2;
      const dd = (d.x - victim.x) ** 2 + (d.y - victim.y) ** 2;
      return dd < db ? d : best;
    });

    reviver.druidBattleReviveCharges = (reviver.druidBattleReviveCharges ?? 1) - 1;
    victim.druidReviveVanishDeath = true;
    const clone = this.spawnDruidBattleReviveClone(victim);
    this.units.push(clone);
    this.unitLayer.addChild(clone.root);
    this.initialAllyHp += clone.maxHp;
    this.currentAllyHp += clone.hp;

    const ir = victim.hitRadiusPx;
    this.druidRevivePillars.push(spawnDruidBattleRevivePillar(this.fxLayer, victim.x, victim.y, ir));
    for (let k = 0; k < 4; k++) {
      this.ringFx.push(
        spawnRingPulse(this.fxLayer, victim.x, victim.y, ir * (1.05 + k * 0.22), 0x4ade80, 0.48 - k * 0.06),
      );
    }
    this.healBursts.push(spawnHealBurst(this.fxLayer, victim.x, victim.y));
    this.floatWords.push(
      spawnFloatNumber(this.floatLayer, victim.x, this.floatAnchorY(victim), '????', 'heal'),
    );
    this.logBattleSkill(
      'druid_bond15_battle_revive',
      reviver,
      `? ${this.unitDebugLabel(victim)} clone=#${clone.unitId} hp=${clone.hp}/${clone.maxHp}`,
    );
    this.applyUnitCollisionSeparation(2);
    return true;
  }

  private spawnDruidBattleReviveClone(source: SimUnit): SimUnit {
    const kind = source.allyKind!;
    const def = ALLY_DEFS[kind];
    const hitDesign = source.hitRadiusPx / LAYOUT_SCALE;
    const moveDesign = source.speedBase / BATTLE_MOVE_SPEED_MULT;
    const reviveHp = Math.max(1, Math.round(source.maxHp * DRUID_BOND15_BATTLE_REVIVE_HP_RATIO));

    const label = source.heroId ? (getHeroDef(source.heroId)?.name ?? def.name) : def.name;
    const mkOpts = {
      allyKind: kind,
      allySourceSlot: source.allySourceSlot,
      bonusCrit: source.bonusCrit,
      hitRadiusDesign: hitDesign,
      heroId: source.heroId,
      knightState: source.knightState,
      knightCooldown: source.knightCooldown,
      knightChargeTargetId: source.knightChargeTargetId,
      knightDeathDenyLeft: source.knightDeathDenyLeft,
      warlockSoulFireCdRem: source.warlockSoulFireCdRem,
      shamanAbilityPhase: source.shamanAbilityPhase,
    };
    ...
    u.maxHp = source.maxHp;
    u.hp = reviveHp;
    u.attackIntervalBase = source.attackIntervalBase ?? source.attackInterval;
    u.skillIds = [...source.skillIds];
    u.hitRadiusPx = source.hitRadiusPx;
    if (source.tokenInnerR != null) u.tokenInnerR = source.tokenInnerR;
    u.root.scale.copyFrom(source.root.scale);

    if (kind === 'druid') {
      u.druidForm = source.druidForm;
      u.druidBearSwipeCount = source.druidBearSwipeCount ?? 0;
      if (source.druidForm === 'bear' && u.tokenDisk && u.tokenLetter && u.tokenInnerR != null) {
        if (u.heroId) {
          swapHeroTokenPortrait(
            { disk: u.tokenDisk, letter: u.tokenLetter },
            u.tokenInnerR,
    ...
    if (kind === 'assassin' && this.bondStacks.assassin >= 6) {
      u.assassinDodgeRem = ASSASSIN_BOND6_DODGE_DURATION_SEC;
    }
  ...
  private castBangBangBomb(attacker: SimUnit, target: SimUnit, nx: number, ny: number): void {
    ...
    const onBombHit = () => {
      ...
      for (const ally of inSplash) {
        const kb = kbMin + this.battleRand() * Math.max(0, kbMax - kbMin);
        this.knockbackAllyFromPoint(ally, cx, cy, kb);
      }
      for (const ally of inSplash) {
  ...
  private fireGyroRetaliationMissile(defender: SimUnit, target: SimUnit): void {
    ...
    const onHit = () => {
      ...
      this.logBattleSkill(SKILL_GYRO_MISSILE_DEFENSE, d, `?? #${tid}`);
    };
  ...
  private pickJetpackWaypoint(fromX: number, fromY: number): { x: number; y: number } {
    ...
    for (let i = 0; i < 24; i++) {
      const ang = this.battleRand() * Math.PI * 2;
      const dist = minD + this.battleRand() * minD * 1.4;
  ...
  private startJetpackBezierSegment(st: BossJetpackAssaultState, boss: SimUnit): void {
    ...
    const px = -dy / len;
    const py = dx / len;
    const off = (30 + this.battleRand() * 20) * LAYOUT_SCALE * (this.battleRand() < 0.5 ? 1 : -1);
    st.cpx = mx + px * off;
    st.cpy = my + py * off;
    st.segT = 0;
  ...
  private jetpackKnockbackAllyAlongTangent(
    ...
    this.floatWords.push(
      spawnFloatNumber(this.floatLayer, ally.x, this.floatAnchorY(ally) - 36, '??', 'magic'),
    );
  }
  ...
  private startBossJetpackAssault(boss: SimUnit): void {
    ...
    this.logBattleSkill(SKILL_JETPACK_ASSAULT, boss, '??????');
    this.pulseKnightHolySanctionCdFromBossCast();
  }
  ...
  private resolveBossOutgoingCritDamage(
    ...
    if (!crit) {
      ...
      if (this.unitHasSkill(boss, 'skill_boss_crit') && this.battleRand() < skillParamNumber(bc, 0, 20) / 100) {
        crit = true;
      } else {
        const bm = getSkillById('skill_blademaster_crit');
        if (this.unitHasSkill(boss, 'skill_blademaster_crit') && this.battleRand() < skillParamNumber(bm, 0, 0.35)) {
          crit = true;
        } else if (this.unitHasSkill(boss, 'skill_normal_crit')) {
          const nc = getSkillById('skill_normal_crit');
          if (this.battleRand() < skillParamNumber(nc, 0, 20) / 100) crit = true;
        }
      }
    }
    let dmg = baseDmg;
  ...
  private pickVanishAmbushTarget(exclude: Set<number>): SimUnit | null {
    let best: SimUnit | null = null;
    for (const a of this.alive('ally')) {
  ...
  private startBossVanishAmbush(boss: SimUnit): void {
    ...
    if (!def) return;
    const target = this.pickVanishAmbushTarget(new Set());
    if (!target) return;
    ...
    boss.bossSkillCast = {
      ...
      pushR,
      backDist,
      hitTargetIds: new Set([target.unitId]),
      ambushStrikesDone: 0,
      invulnFx,
      ringSpin: 0,
    };
    this.logBattleSkill(SKILL_VANISH_AMBUSH, boss, `????? ? #${target.unitId}`);
    this.pulseKnightHolySanctionCdFromBossCast();
  }

  /** @returns ??????????? */
  private dealVanishAmbushStrike(boss: SimUnit, st: BossVanishAmbushState, target: SimUnit): boolean {
    const isCrit = this.isRangedAttacker(target);
    ...
    if (target.hitFlashOverlay && hpBefore > target.hp) {
      target.hitFlashT = HIT_FLASH_DUR * (isCrit ? 3.2 : 2.4);
    }
    for (let k = 0; k < (isCrit ? 4 : 2); k++) {
      this.hitSparks.push(spawnHitSparkBurst(this.fxLayer, target.x + (k - 1) * 6 * LAYOUT_SCALE, target.y));
    }
    const pushDist = Math.round(st.pushR * (0.92 + this.battleRand() * 0.2));
    for (const a of this.alive('ally')) {
      if (a.unitId === target.unitId) continue;
  ...
  private applyVanishAmbushImpact(boss: SimUnit, st: BossVanishAmbushState): void {
    ...
    let lastFxTarget: SimUnit | null = null;
    let lastFxCrit = false;
    const innerR = boss.tokenInnerR ?? boss.hitRadiusPx;

    while (st.ambushStrikesDone < VANISH_AMBUSH_MAX_CHAIN) {
      const target = this.byId(st.targetId);
      if (!target || target.dead) break;
      this.positionBossBehindAlly(boss, target, st.backDist);
      this.syncUnitRootFromStance(boss);
      st.hitTargetIds.add(target.unitId);
      const strikeNo = st.ambushStrikesDone + 1;
      const killed = this.dealVanishAmbushStrike(boss, st, target);
      st.ambushStrikesDone = strikeNo;
      lastFxTarget = target;
      lastFxCrit = this.isRangedAttacker(target);
      this.logBattleSkill(
        SKILL_VANISH_AMBUSH,
        boss,
        `??#${strikeNo} ? #${target.unitId}${killed ? ' ??' : ''}`,
      );
      if (!killed || st.ambushStrikesDone >= VANISH_AMBUSH_MAX_CHAIN) break;
      const next = this.pickVanishAmbushTarget(st.hitTargetIds);
      if (!next) break;
      st.targetId = next.unitId;
    }

    if (lastFxTarget) {
      st.ambushFx = spawnVanishAmbushStrike(
        this.fxLayer,
        lastFxTarget.x,
        lastFxTarget.y,
        lastFxTarget.hitRadiusPx,
        lastFxCrit,
      );
    } else {
  ...
  private beginDefaultAttack(u: SimUnit, target: SimUnit, dist: number, dx: number, dy: number): void {
    ...
    if (u.side === 'enemy') {
      const bc = getSkillById('skill_boss_crit');
      if (this.unitHasSkill(u, 'skill_boss_crit') && this.battleRand() < skillParamNumber(bc, 0, 20) / 100) {
        dmg *= skillParamNumber(bc, 1, 3);
        enemyTag = 'crit';
      } else {
        const bm = getSkillById('skill_blademaster_crit');
        if (this.unitHasSkill(u, 'skill_blademaster_crit') && this.battleRand() < skillParamNumber(bm, 0, 0.35)) {
          dmg *= skillParamNumber(bm, 1, 2);
          enemyTag = 'crit';
        } else if (this.unitHasSkill(u, 'skill_normal_crit')) {
          const nc = getSkillById('skill_normal_crit');
          const pCrit = skillParamNumber(nc, 0, 20) / 100;
          if (this.battleRand() < pCrit) {
    ...
    if (this.isRangedAttacker(u)) {
      this.queueProjectile(u, tid, () => {
        ...
        const t = this.byId(tid);
        if (!a || !t) return;
        if (a.side === 'ally' || this.usesAllyClassAI(a)) {
      ...
      if (u.side === 'ally' || this.usesAllyClassAI(u)) {
  ...
  /**
   * skill_hot_strike????????????????? params[0] ????????????? (1+params[0]) ??????
   * ????????????????? + margin????????????????????
   */
  ...
  private syncBossBerserkFromHp(u: SimUnit): void {
    ...
    if (stage !== prev) {
      ...
      if (stage === 1) {
        this.floatWords.push(spawnFloatNumber(this.floatLayer, u.x, fy, '??', 'crit'));
  ...
  private nearestEnemy(from: SimUnit): SimUnit | null {
    ...
    for (const u of this.units) {
      if (u.dead || u.invincible || !this.isHostileUnits(from, u)) continue;
  ...
  private farthestEnemy(from: SimUnit): SimUnit | null {
    ...
    let bestD = -1;
    for (const u of this.units) {
      if (u.dead || !this.isHostileUnits(from, u)) continue;
      const d = Math.hypot(u.x - from.x, u.y - from.y);
      if (d > bestD) {
        bestD = d;
        best = u;
  ...
  private nearestAlly(from: SimUnit): SimUnit | null {
    ...
    for (const u of this.units) {
      if (u.dead) continue;
      if (this.isArenaPvp) {
        if (this.pvpTeam(u) !== 'attacker') continue;
      } else if (u.side !== 'ally') {
        continue;
      }
      const d = Math.hypot(u.x - from.x, u.y - from.y);
  ...
  private pickPriestHealTarget(u: SimUnit): { target: SimUnit; inRange: boolean } | null {
    ...
    const team = this.pvpTeam(u);
    const others = this.aliveOnTeam(team).filter((a) => a.unitId !== u.unitId && this.allyNeedsHeal(a));

    if (others.length) {
      if (this.bondStacksFor(u).priest >= 15) {
        let worst = others[0]!;
  ...
  private applyHeal(target: SimUnit, amount: number, src?: SimUnit, opts?: { silent?: boolean }): void {
    ...
    if (src?.allyKind === 'priest' && this.bondStacksFor(src).priest >= 6) {
  ...
  private tryCastKnightHeroHolySanction(hero: SimUnit, target: SimUnit): boolean {
    ...
    this.floatWords.push(
      spawnFloatNumber(this.floatLayer, target.x, this.floatAnchorY(target) - 38, '????', 'magic'),
    );
    this.ringFx.push(
  ...
  private applyDamage(target: SimUnit, amount: number, ctx?: DamageCtx): void {
    ...
    if (target.invulnerable && this.pvpTeam(target) === 'attacker' && !this.knightIsCharging(target)) return;

    const atk = ctx?.attacker;
    const hostile = atk != null && !atk.dead && this.isHostileUnits(atk, target);
    const atkTeam = atk ? this.pvpTeam(atk) : null;
    const tgtTeam = this.pvpTeam(target);

    if (
      target.allyKind === 'assassin' &&
      (target.assassinDodgeRem ?? 0) > 0 &&
      hostile &&
      this.battleRand() < ASSASSIN_BOND6_DODGE_CHANCE
    ) {
      this.floatWords.push(
        spawnFloatNumber(this.floatLayer, target.x, this.floatAnchorY(target) - 28, '??', 'buff'),
      );
      return;
    }
    ...
    if (
      ...
      const pool = this.aliveOnTeam('attacker').filter((u) => u.allyKind === 'warrior');
      if (pool.length > 1) {
    ...
    let amt = amount;
    if (hostile && atkTeam === 'attacker' && tgtTeam === 'defender' && atk) {
      amt *= this.run.damageDealtMultAllies;
      if (this.unitHasSkill(target, 'skill_ultralisk_mage_fragile') && atk.allyKind === 'mage') {
        const ul = getSkillById('skill_ultralisk_mage_fragile');
        amt *= skillParamNumber(ul, 0, 2);
      }
      amt *= this.enemyClassWeaknessMult(target, atk);
      ...
      if (
        atk &&
        this.bondStacksFor(atk).mage >= 10 &&
        target.enemyCreatureType === '??'
      ) {
        amt *= 1 + MAGE_BOND_ELEMENT_DAMAGE_BONUS;
      }
      if (target.bossId && target.bossSkillCast?.kind === 'jetpack_assault') {
        amt *= 0.5;
      }
    }
    if (hostile && atkTeam === 'defender' && tgtTeam === 'attacker' && atk) {
      amt *= this.run.damageTakenMultAllies;
      if ((atk.evilFrenzyBuffT ?? 0) > 0 && this.unitHasSkill(atk, 'skill_evil_strenth')) {
        const ev = getSkillById('skill_evil_strenth');
        amt *= 1 + skillParamNumber(ev, 2, 50) / 100;
      }
      amt *= this.enemyDefiasFeverDamageMult(atk);
      if (
        target.allyKind === 'priest' &&
        this.run.priestAllyProtection &&
        this.aliveOnTeam('attacker').some((a) => a.allyKind !== 'priest')
      ) {
        amt *= 0.5;
      }
      if (target.druidForm === 'bear' && target.allyKind === 'druid' && this.bondStacksFor(target).druid >= 3) {
        amt *= DRUID_BEAR_DAMAGE_RETAIN;
      }
      if (this.knightBond3ChargeMitigating(target)) {
        amt *= KNIGHT_BOND3_CHARGE_DAMAGE_RETAIN;
      }
    }
    ...
    if (
      ...
      if (showFloat) {
        this.floatWords.push(
          spawnFloatNumber(this.floatLayer, target.x, this.floatAnchorY(target) - 24, '??', 'shield'),
        );
      }
      return;
    }
    ...
    if (
      ...
      const ws = this.bondStacksFor(target).warrior;
      ...
      if (canTryBlock) {
        ...
        if (this.battleRand() < warriorBondBlockChance(ws, extraChance)) {
          const before = amt;
    ...
    if (this.usesAllyClassAI(target) && hostile) {
    ...
    if (
      ...
      this.endKnightCharge(target, { cooldown: false });
    ...
    const druidReviveSubstituted =
      target.hp <= 0 && target.side === 'ally' && target.allyKind
        ? this.tryDruidBattleRevive(target)
        : false;
    if (target.hp <= 0) {
      target.hp = 0;
      target.dead = true;
      ...
      if (!target.bossId) {
        ...
        if (druidReviveSubstituted || target.druidReviveVanishDeath) {
          target.druidReviveVanishDeath = true;
          target.boomSkipDeathAnim = true;
          target.root.visible = false;
        } else if (this.unitHasSkill(target, 'skill_boom')) {
          ...
          target.deathAnim = {
            ...
            vy: launch.vy,
            g: launch.g,
            spin: (this.battleRand() < 0.5 ? -1 : 1) * (40 + this.battleRand() * 32),
            trailTimer: 0,
          };
    ...
    if (
      ...
      const kb = Math.round((20 + this.battleRand() * 30) * LAYOUT_SCALE);
    ...
    if (lost > 0 && showFloat) {
      ...
      } else if (this.battleRand() < 0.45) {
        this.hitSparks.push(spawnHitSparkBurst(this.floatLayer, target.x, target.y));
      }
    }
    if (blockedLabel && showFloat && target.allyKind === 'warrior') {
      this.floatWords.push(
        spawnFloatNumber(this.floatLayer, target.x, this.floatAnchorY(target) - 28, '??', 'block'),
      );
    }
  ...
  private knightBond3ChargeMitigating(u: SimUnit): boolean {
    return (
      u.allyKind === 'knight' &&
      this.bondStacksFor(u).knight >= 3 &&
      (this.knightIsCharging(u) || (u.knightChargeDrRem ?? 0) > 0)
    );
  }

  private endKnightCharge(u: SimUnit, opts?: { cooldown?: boolean }): void {
    const wasCharging = u.knightState === 'charge';
    u.knightState = 'fight';
    u.collisionDisabled = false;
    ...
    if (wasCharging && this.bondStacksFor(u).knight >= 3) {
      u.knightChargeDrRem = KNIGHT_BOND3_CHARGE_DR_TAIL_SEC;
    }
  }
  ...
  private interruptNonBossEnemyCastFromControl(target: SimUnit): boolean {
    ...
    if (target.defiasBandageChannel) {
      ...
      this.floatWords.push(
        spawnFloatNumber(this.floatLayer, target.x, this.floatAnchorY(target) - 32, '??', 'magic'),
      );
      return true;
    }
    ...
    if (st.kind === 'blade_storm_warn' || st.kind === 'blade_storm_channel') {
      this.interruptBossBladeStorm(target, '???????');
      this.floatWords.push(
        spawnFloatNumber(this.floatLayer, target.x, this.floatAnchorY(target) - 32, '??', 'magic'),
      );
      return true;
    ...
    if (st.kind === 'overload_explosion_channel') {
      this.interruptBossOverloadExplosion(target, '???????', false);
      this.floatWords.push(
        spawnFloatNumber(this.floatLayer, target.x, this.floatAnchorY(target) - 32, '??', 'magic'),
      );
      return true;
    ...
    if (st.kind === 'punch_windup' || st.kind === 'rush_windup' || st.kind === 'rhahk_smash_windup') {
      ...
      this.logBattleSkill(st.skillId, target, '?????????');
      this.floatWords.push(
        spawnFloatNumber(this.floatLayer, target.x, this.floatAnchorY(target) - 32, '??', 'magic'),
      );
      return true;
    ...
    if (st.kind === 'rush_charge') {
      ...
      this.logBattleSkill('skill_boss_rush', target, '?????????');
      this.floatWords.push(
        spawnFloatNumber(this.floatLayer, target.x, this.floatAnchorY(target) - 32, '??', 'magic'),
      );
      return true;
  ...
  private snareTrapEffectSec(owner: SimUnit): number {
    const stacks = this.bondStacksFor(owner);
    return stacks.archer >= 6 ? SNARE_TRAP_EFFECT_SEC_BOND6 : SNARE_TRAP_EFFECT_SEC;
  }

  private snareTrapAppliesTo(u: SimUnit): boolean {
    const stacks = this.bondStacksFor(u);
    if (stacks.archer >= 15) return this.isRangedAttacker(u);
    return u.allyKind === 'archer';
  }

  /** ???????????????/??????? */
  private isAllyInEnemyWindupWarn(ally: SimUnit, enemy: SimUnit): boolean {
    const st = enemy.bossSkillCast;
    if (!st) return false;
    const allyR = ally.hitRadiusPx;
    const eR = this.hitRadius(enemy);
    switch (st.kind) {
      case 'punch_windup':
        return allyInPunchSector(
          ally.x,
          ally.y,
          allyR,
          st.cx,
      ...
      case 'rush_windup':
        return (
          distPointToSegment(ally.x, ally.y, st.startX, st.startY, st.endX, st.endY) <= st.halfW + allyR
        );
      case 'rush_charge': {
        const ex = enemy.x + st.dirx * Math.max(st.remainDist, 0);
        const ey = enemy.y + st.diry * Math.max(st.remainDist, 0);
        return distPointToSegment(ally.x, ally.y, st.prevX, st.prevY, ex, ey) <= st.halfW + allyR;
      }
      case 'rhahk_smash_windup':
        return Math.hypot(ally.x - st.cx, ally.y - st.cy) <= st.rMax + allyR;
      case 'blade_storm_warn': {
        const def = getSkillById(SKILL_BLADE_STORM);
        const radius = skillParamDesignPx(def, 4, 100);
        return Math.hypot(ally.x - enemy.x, ally.y - enemy.y) <= radius + allyR + eR;
      }
      case 'blade_storm_channel':
        return Math.hypot(ally.x - enemy.x, ally.y - enemy.y) <= st.radius + allyR + eR;
      case 'overload_explosion_channel':
        return Math.hypot(ally.x - enemy.x, ally.y - enemy.y) <= st.radius + allyR + eR;
      default:
        return false;
    }
  ...
  private findEnemyWindupThreatForArcher(ally: SimUnit): SimUnit | null {
    let best: SimUnit | null = null;
    let bestD = Number.POSITIVE_INFINITY;
    for (const e of this.alive('enemy')) {
      if (!this.isAllyInEnemyWindupWarn(ally, e)) continue;
  ...
  private moveArcherArcDodge(u: SimUnit, centerX: number, centerY: number, stepLen: number): void {
    let ox = u.x - centerX;
    let oy = u.y - centerY;
    const od = Math.hypot(ox, oy) || 1;
    ox /= od;
    oy /= od;
    const sign = u.unitId % 2 === 0 ? 1 : -1;
    const tx = -oy * sign;
    const ty = ox * sign;
    const ir = this.unitTokenRootYOffsetPx(u);
    let cand = this.clampAllyKnockbackXY(u.x + tx * stepLen, u.y + ty * stepLen, ir);
    if (Math.hypot(cand.x - u.x, cand.y - u.y) < stepLen * 0.08) {
      cand = this.clampAllyKnockbackXY(u.x - tx * stepLen, u.y - ty * stepLen, ir);
    }
    u.x = cand.x;
    u.y = cand.y;
    this.syncUnitRootFromStance(u);
  ...
  private tryArcherAttackInRange(
    u: SimUnit,
    target: SimUnit,
    dist: number,
    dx: number,
  ...
  private tryArcherBond10Move(
    ...
    if (this.bondStacksFor(u).archer < 10 || !this.usesAllyClassAI(u) || u.allyKind !== 'archer') return false;

    const threat = this.findEnemyWindupThreatForArcher(u);
    if (threat) {
      this.moveArcherArcDodge(u, threat.x, threat.y, stepLen);
      this.tryArcherAttackInRange(u, target, dist, dx, dy, dt, aimDist, meleeStuck);
      return true;
    }

    const retreatLine =
      this.weaponReachPx(u) - Math.round(ARCHER_BOND10_KITE_RETREAT_MARGIN_DESIGN * LAYOUT_SCALE);
    if (dist < retreatLine) {
      const ir = this.unitTokenRootYOffsetPx(u);
      const cand = this.clampAllyKnockbackXY(u.x - nx * stepLen, u.y - ny * stepLen, ir);
      u.x = cand.x;
      u.y = cand.y;
      this.syncUnitRootFromStance(u);
      this.tryArcherAttackInRange(u, target, dist, dx, dy, dt, aimDist, meleeStuck);
  ...
  private applyBattleOpenArcherSnareTrap(): void {
    const teams: Array<'attacker' | 'defender'> = this.isArenaPvp ? ['attacker', 'defender'] : ['attacker'];
    for (const team of teams) {
      const caster = this.aliveOnTeam(team).find((u) => isArcherSnareTrapHero(u.heroId));
      if (!caster) continue;
      for (const a of this.aliveOnTeam(team)) {
  ...
  private tickArcherSnareTrapBuffs(dt: number): void {
    for (const u of this.units) {
      if (u.dead || !u.snareTrapBuff) continue;
      const b = u.snareTrapBuff;
      if (!b) continue;
  ...
  private trySnareTrapStun(
    trapOwner: SimUnit,
    buff: NonNullable<SimUnit['snareTrapBuff']>,
    attacker: SimUnit,
  ): void {
    if (this.bondStacksFor(trapOwner).archer < 10) return;
  ...
  private applyArcherSnareTrapMitigate(target: SimUnit, amt: number, ctx?: DamageCtx): number {
    ...
    if (!b) return amt;
    if (!ctx?.attacker || !this.isHostileUnits(target, ctx.attacker)) return amt;
    if (!this.isPhysicalDamage(ctx)) return amt;

    if (b.activeRemainSec > 0) {
      this.trySnareTrapStun(target, b, ctx.attacker);
      this.syncSnareTrapGfxVisual(target);
      return 1;
    }
    if (b.armedRemainSec > 0) {
      b.activeRemainSec = this.snareTrapEffectSec(target);
      this.trySnareTrapStun(target, b, ctx.attacker);
  ...
  private dealAllyHit(u: SimUnit, target: SimUnit, baseDmg: number, ctx?: DamageCtx): void {
    const bonds = this.bondStacksFor(u);
    let dmg = baseDmg * this.allyOutgoingDamageMult(u);
    if (u.allyKind === 'archer') {
      const cap = archerBondFocusCap(bonds.archer);
    ...
    if (u.allyKind === 'assassin' && bonds.assassin >= 3) critP += ASSASSIN_BOND3_CRIT_BONUS;
    if (this.battleRand() < critP) {
    ...
    if (u.allyKind === 'assassin' && u.assassinBlinkStunNext && !target.dead && !target.bossId) {
      ...
      if (this.usesAllyClassAI(target)) {
        this.applyAllyStun(target, ASSASSIN_BOND10_BLINK_STUN_SEC);
      } else {
    ...
    if (u.allyKind === 'mage') {
      const splashR = this.mageSplashRadiusPx(u);
  ...
  /**
   * ???**??**?????????????????????? `MULAN_WHIRL_BLADE_OUTSIDE_HP_RING_DESIGN_PX`??? px??
   * ??? `mulanWhirlwindOuterRadiusPx`??????????????????????
   */
  ...
  private maybeMulanWhirlwindAfterAllyHit(u: SimUnit, _primary: SimUnit): void {
    ...
    for (const e of this.aliveHostileTo(u)) {
      ...
      this.logDevBattleTest(`?????? entity=${e.unitId} ${rounded}`);
    ...
    if (!geom) {
      this.logDevBattleTest(`???????(??) ?????=${hits.length}`);
    ...
    for (let i = this.mulanWhirlRings.length - 1; i >= 0; i--) {
      ...
      while (r.damageFloatEmitted < 10 && r.t + 1e-9 >= (r.damageFloatEmitted + 1) * 0.1) {
        ...
        for (const h of r.hits) {
          ...
          this.logDevBattleTest(`?????? ${k + 1}/10 entity=${h.enemyUnitId} ${chunk}`);
          this.floatWords.push(
            spawnFloatNumber(
              this.floatLayer,
              e.x + jx,
  ...
  private maybeDoubleShotArcher(u: SimUnit, target: SimUnit, baseDmg: number): void {
    const p = archerBondDoubleShotChance(this.bondStacksFor(u).archer);
    if (u.allyKind !== 'archer' || p <= 0) return;
    if (this.battleRand() >= p) return;
  ...
  private tickMeteor(dt: number): void {
    this.tickMeteorForTeam(dt, 'attacker');
    if (this.isArenaPvp) this.tickMeteorForTeam(dt, 'defender');
  }

  private tickMeteorForTeam(dt: number, team: 'attacker' | 'defender'): void {
    const stacks = this.bondStacksFor(team);
    if (stacks.mage < 15) return;
    const mages = this.aliveOnTeam(team).filter((u) => u.allyKind === 'mage');
    if (!mages.length) return;
    const cdRef = team === 'attacker' ? 'meteorCd' : 'meteorCdDefender';
    this[cdRef] -= dt;
    if (this[cdRef] > 0) return;
    this[cdRef] = MAGE_BOND_METEOR_INTERVAL_SEC;
    const enemies = this.units.filter((u) => !u.dead && this.pvpTeam(u) !== team);
    if (!enemies.length) return;
    const epicenter = enemies[Math.floor(this.battleRand() * enemies.length)]!;
    ...
    if (this.run.mageMeteorCrits && this.battleRand() < this.run.mageCritChance) {
      meteorTag = 'crit';
    }
  ...
  private tickKnightCharge(u: SimUnit, dt: number): void {
    ...
    if (dist <= chargeHitDist) {
      ...
      let dmgHit = u.atk * KNIGHT_CHARGE_HIT_DAMAGE_MULT;
      ...
      if (this.bondStacksFor(u).knight >= 6 && !tgt.bossId && !tgt.dead) {
        if (this.usesAllyClassAI(tgt)) {
          this.applyAllyStun(tgt, KNIGHT_BOND_CHARGE_STUN_SEC);
        } else {
  ...
  private tickPriest(u: SimUnit, dt: number, targetEnemy: SimUnit | null): void {
    ...
    const priestStacks = this.bondStacksFor(u).priest;
    let healAmt = u.atk * priestBondHealCoeff(priestStacks);
    ...
    if (healPick) {
      ...
      if (healPick.inRange || priestStacks >= 15) {
        u.cd -= dt;
        if (u.cd <= 0) {
  ...
  private startBossRhahkSmashWindup(boss: SimUnit): void {
    ...
    this.logBattleSkill('skill_rhahk_smash', boss, `???? ? #${primary.unitId}`);
    this.pulseKnightHolySanctionCdFromBossCast();
  }
  ...
  private applyRhahkSmashImpact(boss: SimUnit, st: BossRhahkSmashWindupState): void {
    ...
    this.bossPutSkillOnCooldown(boss, 'skill_rhahk_smash');
    this.logBattleSkill('skill_rhahk_smash', boss, any ? '????' : '???');
  ...
  private castRhahkWarcry(boss: SimUnit): void {
    ...
    this.floatWords.push(
      spawnFloatNumber(this.floatLayer, boss.x, this.floatAnchorY(boss) - 44, '??', 'buff'),
    );
    this.floatWords.push(
    ...
    this.logBattleSkill(
      'skill_rhahk_warcry',
      boss,
      `??=${boss.rhahkWarcryStacks} ?=${boss.atk} ??=${heal}`,
    );
  }
  ...
  private maybeStackPoisonStrike(attacker: SimUnit, target: SimUnit): void {
    ...
    if (stacks === 1 || stacks % 5 === 0) {
      this.logBattleSkill(SKILL_POISON_STRIKE, attacker, `? #${target.unitId} ?=${stacks}`);
    }
  }
  ...
  private startBossBladeStormWarn(boss: SimUnit): void {
    ...
    this.logBattleSkill(SKILL_BLADE_STORM, boss, '??');
    this.pulseKnightHolySanctionCdFromBossCast();
  }
  ...
  private beginBossBladeStormChannel(boss: SimUnit): void {
    ...
    this.restoreBossTokenRingAfterBladeWarn(boss);
    this.logBattleSkill(SKILL_BLADE_STORM, boss, `???? r=${radius}`);
  }
  ...
  private castBossBlinkFan(boss: SimUnit): void {
    ...
    this.logBattleSkill(SKILL_BLINK_FAN, boss, `?? (${Math.round(dest.x)},${Math.round(dest.y)})`);
    this.pulseKnightHolySanctionCdFromBossCast();
  }
  ...
  private fireBossBlinkFanKnives(boss: SimUnit, volley: BossBlinkFanVolleyState): void {
    ...
    if (allies.length) {
      this.logBattleSkill(SKILL_BLINK_FAN, boss, `?? hit=${allies.length}`);
    }
  }
  ...
  private startBossPunchWindup(boss: SimUnit): void {
    ...
    this.logBattleSkill('skill_boss_punch', boss, '????');
    this.pulseKnightHolySanctionCdFromBossCast();
  }
  ...
  private startBossRushWindup(boss: SimUnit): void {
    ...
    this.logBattleSkill('skill_boss_rush', boss, '????');
    this.pulseKnightHolySanctionCdFromBossCast();
  }
  ...
  private castTaurenStomp(boss: SimUnit): void {
    ...
    this.logBattleSkill(SKILL_TAUREN_STOMP, boss, hit ? `??${hit}` : '???');
    this.pulseKnightHolySanctionCdFromBossCast();
  }
  ...
  private castTaurenShockwave(boss: SimUnit): void {
    ...
    this.logBattleSkill(SKILL_TAUREN_SHOCKWAVE, boss, hit ? `??${hit}` : '???');
    this.pulseKnightHolySanctionCdFromBossCast();
  }
  ...
  /**
   * ?????? {@link battlePurgeDispel} ? **green** ??? debuff?
   * ?/??????????????????
   */
  ...
  /**
   * ?????? {@link battlePurgeDispel} ? **green** ??? buff?
   */
  ...
  private grantOverloadExplosionChannelShield(boss: SimUnit): void {
    const add = Math.max(1, Math.round(boss.maxHp * OVERLOAD_EXPLOSION_CHANNEL_SHIELD_MAX_HP_RATIO));
  ...
  private interruptBossOverloadExplosion(boss: SimUnit, reason: string, shieldBroken: boolean): void {
    ...
    if (shieldBroken) {
      ...
      this.floatWords.push(
        spawnFloatNumber(this.floatLayer, boss.x, this.floatAnchorY(boss) - 40, '????', 'magic'),
      );
    }
  ...
  private applyBossOverloadExplosionImpact(boss: SimUnit, st: BossOverloadExplosionChannelState): void {
    ...
    this.floatWords.push(
      spawnFloatNumber(this.floatLayer, boss.x, this.floatAnchorY(boss) - 48, '??', 'crit'),
    );
    this.logBattleSkill(SKILL_OVERLOAD_EXPLOSION, boss, `????=${hits}`);
  }
  ...
  private startBossOverloadExplosion(boss: SimUnit): void {
    ...
    this.logBattleSkill(SKILL_OVERLOAD_EXPLOSION, boss, `???? ??=${boss.shield ?? 0}`);
    this.pulseKnightHolySanctionCdFromBossCast();
  }
  ...
  private tickBossOverloadExplosionChannel(boss: SimUnit, st: BossOverloadExplosionChannelState, dt: number): void {
    ...
    if (st.channelHadShield && (boss.shield ?? 0) <= 0) {
      this.interruptBossOverloadExplosion(boss, '???????', true);
      return;
    }
  ...
  private castBossOverloadLaser(boss: SimUnit): void {
    ...
    if (killed) {
      ...
      this.logBattleSkill(SKILL_OVERLOAD_LASER, boss, `?? #${target.unitId} ??=${powerPct}% CD=${halfCd}s`);
    } else {
      ...
      this.logBattleSkill(
        SKILL_OVERLOAD_LASER,
        boss,
        `? #${target.unitId} dmg=${dmg} hp ${prevHp}?${target.hp} ????=${basePct + boss.overloadLaserPowerBonus}%`,
      );
    }
  ...
  private tickMechanoPioneerPassive(u: SimUnit): void {
    ...
    this.logBattleSkill(SKILL_MECHANO_PIONEER, u, '????????');
  }
  ...
  private applyBossPunchImpact(boss: SimUnit, st: BossPunchWindupState): void {
    ...
    this.logBattleSkill('skill_boss_punch', boss, any ? '????' : '???');
  ...
  private knockbackAllyFromRushHit(a: SimUnit, footX: number, footY: number, bossRadius: number): void {
    ...
    const nx = dx / d;
    const ny = dy / d;
    const wantDist = bossRadius * (1 + this.battleRand() * 2);
    const ir = this.unitTokenRootYOffsetPx(a);
    ...
    this.syncUnitMoveSpeed(a);
    this.floatWords.push(spawnFloatNumber(this.floatLayer, a.x, this.floatAnchorY(a) - 40, '??', 'magic'));
    this.hitSparks.push(spawnHitSparkBurst(this.fxLayer, a.x, a.y));
  }
  ...
  private tickBossSkillCast(boss: SimUnit, dt: number): void {
    ...
    if (st.kind === 'blade_storm_warn' || st.kind === 'blade_storm_channel') {
      if ((boss.stunT ?? 0) > 0) {
        this.interruptBossBladeStorm(boss, '????');
        return;
      }
    }
    ...
    if (st.kind === 'rhahk_smash_windup') {
      ...
      if (!primary || primary.dead) {
        this.interruptRhahkSmash(boss, st, '????????');
        return;
      }
      st.t += dt;
  ...
  private tickBattleScreenShake(dt: number): void {
    ...
    this.position.set((this.battleRand() - 0.5) * 2 * mag, (this.battleRand() - 0.5) * 2 * mag);
  }
  ...
  private mountBattleStatsPanel(): void {
    ...
    this.statsTabDamageLbl = mkTab('??', false, Math.round(14 * LAYOUT_SCALE));
    this.statsTabHealLbl = mkTab('??', true, Math.round(78 * LAYOUT_SCALE));
  ...
  private syncBattleStatsPanelLayout(): void {
    ...
    this.statsHeaderLabel.text = this.statsPanelExpanded ? '??' : '????';
  ...
  private refreshHudTimerAndHpBars(): void {
    this.hudTimer.text = `?????${Math.max(0, this.timeLeft).toFixed(1)}s / ${this.timeLimit}s`;
    ...
    this.hudAllyLabel.text = `????? ${Math.round(this.currentAllyHp)} / ${this.initialAllyHp}`;
    ...
    this.hudEnemyLabel.text = `????? ${Math.round(this.currentEnemyHp)} / ${this.initialEnemyHp}`;
    ...
    this.hudEnemyBar
      ...
      .fill(this.isArenaPvp ? 0xef4444 : 0x3b82f6);
  }
  ...
  private updateFrame(dt: number): void {
    ...
    this.assassinBlinkClaimedTargetIds.clear();

    if (this.shamanBondOpeningBloodlustDelayRem > 0) {
      this.shamanBondOpeningBloodlustDelayRem = Math.max(0, this.shamanBondOpeningBloodlustDelayRem - dt);
      if (this.shamanBondOpeningBloodlustDelayRem <= 0) {
        this.castShamanBondOpeningBloodlust(this.units, 'attacker');
      }
    }
    if (this.shamanBondOpeningBloodlustDelayRemDefender > 0) {
      this.shamanBondOpeningBloodlustDelayRemDefender = Math.max(
        0,
        this.shamanBondOpeningBloodlustDelayRemDefender - dt,
      );
      if (this.shamanBondOpeningBloodlustDelayRemDefender <= 0) {
        this.castShamanBondOpeningBloodlust(this.units, 'defender');
      }
    }
    ...
    tickDruidBattleRevivePillars(this.druidRevivePillars, dt);
    ...
    for (const u of this.units) {
      ...
      if (u.allyKind === 'knight' && (u.knightChargeDrRem ?? 0) > 0) {
        u.knightChargeDrRem = Math.max(0, (u.knightChargeDrRem ?? 0) - dt);
      }
      ...
      if ((u.fearT ?? 0) > 0 && this.isArenaNpcEnemy(u)) {
      ...
      if (this.usesAllyClassAI(u) && u.allyKind === 'assassin') {
        if ((u.assassinBlinkCdRem ?? 0) > 0) {
          u.assassinBlinkCdRem = Math.max(0, (u.assassinBlinkCdRem ?? 0) - dt);
        }
        if (this.tickAssassinVanish(u, dt)) continue;
      }
      ...
      if (this.isArenaNpcEnemy(u) && this.unitHasSkill(u, 'skill_dread_warrior_assault') && !u.dreadAssaultUsed) {
        const dw = getSkillById('skill_dread_warrior_assault');
        const rAss = skillParamDesignPx(dw, 0, 108);
        const near = this.aliveOnTeam('attacker').filter((a) => Math.hypot(a.x - u.x, a.y - u.y) <= rAss);
        if (near.length) {
      ...
      if (
        ...
        while ((u.bossBerserkSparkAcc ?? 0) >= interval) {
          ...
          const ox = (this.battleRand() - 0.5) * u.hitRadiusPx * 1.4;
          const oy = (this.battleRand() - 0.5) * u.hitRadiusPx * 0.55;
      ...
      if (this.usesAllyClassAI(u) && u.allyKind === 'priest') {
        const te = this.nearestEnemy(u);
        this.tickPriest(u, dt, te);
        continue;
      }

      if (this.usesAllyClassAI(u) && u.allyKind === 'shaman') {
        this.tickShamanAlly(u, dt, kbActive);
        continue;
      }

      if (this.usesAllyClassAI(u) && u.allyKind === 'assassin' && this.tryAssassinBlinkStrike(u)) {
        continue;
      }

      if (
        this.usesAllyClassAI(u) &&
        u.heroId &&
        isMageArcaneMissilesHero(u.heroId) &&
        this.tickHeroMageArcaneMissiles(u, dt, kbActive)
      ) {
        continue;
      }

      if (this.usesAllyClassAI(u) && u.allyKind === 'knight' && u.knightState === 'fight') {
      ...
      if (this.isArenaNpcEnemy(u) && this.tickDefiasBandage(u, dt)) {
        continue;
      }

      if (this.isArenaNpcEnemy(u) && this.tickEnemyVoidWalk(u, dt)) {
        continue;
      }

      if (this.isArenaNpcEnemy(u) && this.tickEnemyBacklineLeap(u, dt)) {
        continue;
      }
      ...
      if (!kbActive) {
        ...
        const meleeStuck =
          ...
          dist <= aimDist + Math.max(18, Math.round(24 * LAYOUT_SCALE));
        if (
          this.tryArcherBond10Move(u, target, dt, dist, dx, dy, nx, ny, stepLen, aimDist, meleeStuck)
        ) {
          // ????????? / ????????????
        } else if (dist <= aimDist || meleeStuck) {
          u.cd -= dt;
          if (u.cd <= 0) {
            this.beginDefaultAttack(u, target, dist, dx, dy);
          }
